...
const tests = JSON.parse(readFileSync(resolve(actual, "tests.json"), "utf8"))
if (tests.length !== 164 || new Set(tests.map((t) => t.path)).size !== 116)
  throw Error("Incomplete upstream test execution")
const result = spawnSync("node", ["tests/parity/run.mjs"], {
  cwd: root,
  stdio: "inherit",
  env: process.env,
})
...
process.exitCode = result.status || reference.status || 0
